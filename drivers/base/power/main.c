...
static int device_resume(struct device *dev, pm_message_t state)
{
    ...
    /* FIH, ChandlerKang, 10/02/01 */ 
	#ifdef CONFIG_FIH_FXX
    if( (dev->class && dev->class->resume ) || (dev->bus && dev->bus->resume ) ){
        printk(KERN_INFO "PM: %s(): %s\n", __FUNCTION__, dev_driver_string(dev));
    }
    #endif 
...
static int device_suspend(struct device *dev, pm_message_t state)
{
	...
	/* FIH, ChandlerKang, 10/02/01 */ 
    #ifdef CONFIG_FIH_FXX
    if( (dev->class && dev->class->suspend ) || (dev->bus && dev->bus->suspend ) ){
        printk(KERN_INFO "PM: %s(): %s\n", __FUNCTION__, dev_driver_string(dev));
    }
    #endif
