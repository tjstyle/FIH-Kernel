...
// +++ FIH, KarenLiao, @20100104: [F0X.B-9394]:Add debug mask mechanism for audio driver.
#include <linux/cmdbgapi.h>
extern int fih_audio_debug_mask;
// --- FIH, KarenLiao, @20100104: [F0X.B-9394]:Add debug mask mechanism for audio driver.
...
//+++ FIH, KarenLiao@20090909: FA3.B-1107: [ACC] Playing Video and plug in/unplug HS many times, DUT works abnormality.
	...
	if(bHookSwitchEnabled != HeadsetState)
	{
		if(bHookSwitchEnabled) {
			...
			fih_printk(fih_audio_debug_mask, FIH_DEBUG_ZONE_G3, "Headset: Enable_Headset_Without_Mic == %d\n", EnableAbnormalHS);
			...
			if(EnableAbnormalHS == true){
			    EnableMic = 0;
			    proc_comm_enable_pm_mic(&EnableMic);
				Q7x27_kybd_hookswitch_irqsetup(false);  //FIH, KarenLiao @2010/07/22: [F0XE.B-1501]: [Power Manager] Plug in/out AC charger and USB cable and Wired HS behavior is incorrect.
			}else{
			    Q7x27_kybd_hookswitch_irqsetup(bHookSwitchEnabled);
			}  	
			// --- FIH, KarenLiao, @20091023: [F0X.FC-663]: Allow user to use the normal headset.
		...
		//printk(KERN_INFO "Headset: hook_switch_enabled = %d\n", bHookSwitchEnabled);
		fih_printk(fih_audio_debug_mask, FIH_DEBUG_ZONE_G3, "Headset: hook_switch_enabled = %d\n", bHookSwitchEnabled);
	}
...
*/	
	...
	if(gpio_get_value(data->gpio) == state)
	{
		...
		fih_printk(fih_audio_debug_mask, FIH_DEBUG_ZONE_G3, "Headset: HeadsetInserted = %d\n", data->bHeadsetInserted);
	}
...
//SW2-6-MM-RC-Audio_Porting-00*{
	...
	if (!headset_switch_wq) {
		fih_printk(fih_audio_debug_mask, FIH_DEBUG_ZONE_G3,  "%s: create workque failed \n", __func__);		
		return -EBUSY;
	}
...
//+++ FIH, KarenLiao, @20090731: [F0X.FC-41]: The action of Inserting headset is the wake up action.
	...
	if (ret < 0){
		...
		fih_printk(fih_audio_debug_mask, FIH_DEBUG_ZONE_G3, "Headset: gpio_switch_prob: enable_irq_wake failed for HS detect.\n");
	}
#endif			
//--- FIH, KarenLiao, @20090731: [F0X.FC-41]: The action of Inserting headset is the wake up action.
#endif
