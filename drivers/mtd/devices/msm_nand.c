...
#include <linux/cmdbgapi.h>
    ...
    #define pr_err(fmt, ...)    \
        do  \
        {   \
            fih_printk(msm_nand_debug_mask, FIH_DEBUG_ZONE_G0, KERN_ERR fmt, ##__VA_ARGS__);\
        }while(0)

    #define pr_info(fmt, ...)    \
        do  \
        {   \
            fih_printk(msm_nand_debug_mask, FIH_DEBUG_ZONE_G0, KERN_INFO fmt, ##__VA_ARGS__);\
        }while(0)

    #define pr_debug(fmt, ...)    \
        do  \
        {   \
            fih_printk(msm_nand_debug_mask, FIH_DEBUG_ZONE_G0, KERN_DEBUG fmt, ##__VA_ARGS__);\
        }while(0)

/* +++ FIH, 2009/11/18, paul huang*/
unsigned long nand_flash_id_info = 0;
/* --- FIH, 2009/11/18, paul huang*/
...
uint32_t msm_nand_debug_mask = 0;
module_param_named(
    debug_mask, msm_nand_debug_mask, uint, S_IRUGO | S_IWUSR | S_IWGRP
);
#endif
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	if (dev_found) {
		...
		/* --- FIH, 2009/11/20, paul huang*/
		/* +++ FIH, 2009/11/18, paul huang*/
		nand_flash_id_info = supported_flash[index].flash_id;
		/* --- FIH, 2009/11/18, paul huang*/
...
//	mutex_lock(&mtd_table_mutex);
    len = sprintf(page, "Device name:%s ID:0x%X\n",
        /* FIH, Debbie Sun, 2010/05/24 { */
        //supported_flash[supported_flash_index].device_name,
        flash_name,
        /* FIH, Debbie Sun, 2010/05/24 } */
        supported_flash[supported_flash_index].flash_id);
