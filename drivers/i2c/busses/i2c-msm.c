...
/* [FXX_CR], add recovery and log for i2c */
...
/* FIH, Henry Juang, 2009/08/24 { */
/* [FXX_CR], add recovery and log for i2c on 4515*/
#ifdef CONFIG_FIH_FXX
		printk("***I2C dev->suspended\n");
#endif
/* } FIH, Henry Juang, 2009/08/24 */
		return -EIO;
	}
...
#ifdef CONFIG_FIH_FXX
			{
				printk("*******i2c-msm.c error status=0x%x.slave address=0x%x\n",init_status,addr);
#endif
				ret = msm_i2c_recover_bus_busy(dev, adap);
#ifdef CONFIG_FIH_FXX
				printk("***Finish recovery with status=0x%x\n",readl(dev->base + I2C_STATUS));
			}
#endif
				if (ret) {
					dev_err(dev->dev,
...
/* [FXX_CR], add recovery and log for i2c */
#ifdef CONFIG_FIH_FXX
			printk("***Status=0x%x,reg=0x%x, status=0x%x\n",readl(dev->base + I2C_STATUS),msgs->addr,readl(dev->base + I2C_STATUS));
#endif
/* } FIH, Henry Juang, 2009/06/06 */
			writel(I2C_WRITE_DATA_LAST_BYTE,
				dev->base + I2C_WRITE_DATA);
...
#ifdef CONFIG_FIH_FXX
		printk("******I2C-msm Suspend status=0x%x,GPIO_60=%d,GPIO61=%d\n",readl(dev->base + I2C_STATUS),gpio_get_value(60),gpio_get_value(61));
#endif
/* } FIH, Henry Juang, 2009/06/06 */
	}
...
/* [FXX_CR], add log for i2c */
#ifdef CONFIG_FIH_FXX
		printk("******I2C-msm Resume status=0x%x,GPIO_60=%d,GPIO61=%d\n",readl(dev->base + I2C_STATUS),gpio_get_value(60),gpio_get_value(61));
#endif
/* } FIH, Henry Juang, 2009/06/06 */
	return 0;
}
