...
#include <asm/io.h>
#include <linux/err.h>
#include <linux/clk.h>    //For GP_CLK
...
bool bGP_CLK_on = false;

struct i2c_client *bl_max8831_i2c = NULL;
static struct proc_dir_entry *bl_max8831_proc_file = NULL;

static uint32_t max8831_debug_mask = 0;
module_param_named(debug_mask, max8831_debug_mask, uint, S_IRUGO | S_IWUSR | S_IWGRP);
...
#else
	printk(KERN_ERR "Terry @@ Max8831 : This should be kill  - 2!!\n");
#endif
...
int lcd_bl_set_intensity(int level)
{
	...
	{
		//fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]BL level:%d , reg:%d !!, gProximity_flag_On:%d \n",level,value.reg,gProximity_flag_On);
/* FIH, Henry Juang, 2009/11/20 --*/
		if(level == 0 ) 
		{
                     //printk(KERN_ERR "[FM6]lcd_bl_set_intensity:Open BL %d\n",level);
			fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[FM6]lcd_bl_set_intensity:Open BL %d\n",level);
		
                     writel(0x0, MSM_CLK_CTL_BASE + 0x5c);
                     //Terry for FM6.B-17, FM6.B-149 +++++
                     if(bGP_CLK_on == true)
                     {
                         clk_disable(gp_clk);
                         bGP_CLK_on = false;
			...
			fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]Check LCM on\n");
			// FXX_CR, Neo Chen, 2009.06.12, Add for ensuring backlight on after panel on +++
			while(!checkPanelON)
			{
				...
				if(!checkPanelON)
				{
					...
					fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G1, "[Max8831]LCM On checkTimes: %d\n", checkTimes);
				}
				//FXX_CR, Neo Chen, 2009.12.31 Let backlight not to open until LCM is on
				//if(checkTimes>=20) return -1;
			}
			// FXX_CR, Neo Chen, 2009.06.12 ---
			fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]After checking, LCM is on\n");
                     ...
                     if(level == 100) 
                     {
                         writel((~478 << 16) | 0xb20, MSM_CLK_CTL_BASE + 0x5c);
                         udelay(1);
                     }
                     
                     writel((1U << 16) | (~(level * 478 / 100 ) & 0xffff), MSM_CLK_CTL_BASE + 0x58);
                     writel((~(239)<< 16) | 0xb58, MSM_CLK_CTL_BASE + 0x5c);  //19.2M/4/240=20KHz
                     //printk(KERN_ERR "[FM6]lcd_bl_set_intensity:Open BL %d\n",level);
...
static int max8831_probe(struct i2c_client *client,const struct i2c_device_id *device_id)
{
	...
	printk( KERN_INFO "Driver probe: %s\n", __func__ );
        ...
        if (IS_ERR(gp_clk)) 
        {
                printk(KERN_ERR "[May] GP_CLK_INIT: could not get gp clock\n");
                gp_clk = NULL;
        }
