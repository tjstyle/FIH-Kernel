...
/* [FXX_CR], Add for proximity driver to turn on/off BL and TP. */
	...
	{
		if(level ==0)
			fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]gProximity_flag_On , System set BL level:0\n");
		else
		{
			fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]gProximity_flag_On , System BL level:%d\n",level);
			return 0;
		}
	}
/* FIH, Henry Juang, 2009/11/20 --*/
...
/* FIH, Henry Juang, 2009/11/20 --*/
		if(level == 0 && (value.reg & 0x01)) 
		{
			...
			fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]lcd_bl_set_intensity():close BL %d\n", value.data);
			return ret;
			...
			fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]lcd_bl_set_intensity:Open BL %d\n", value.data);
		}
...
{
	fih_printk(max8831_debug_mask, FIH_DEBUG_ZONE_G0, "[Max8831]Proximity_Flag_Set:pre-value %d\n", gProximity_flag_On);
	if(flag){
