...
/* FIH, Michael Kao, 2010/01/03{ */
/* [FXX_CR], add for debug mask*/
static int tca6507_debug_mask;
module_param_named(debug_mask, tca6507_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
/* FIH, Michael Kao, 2010/01/03{ */
...
static int tca6507_read( struct i2c_client *client, u8 *rxdata, int length )
{
	...
	if ( 0 > ret ) {
		fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "i2c rx failed %d\n", ret );
		return -EIO;
	}
...
static int tca6507_write( struct i2c_client *client, u8 *txdata, int length )
{
	...
	if ( 0 > ret ) {
		fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "i2c tx failed %d\n", ret );
		return -EIO;
	}
...
void tca6507_led_blink(int led_selection, uint8_t *cmd)
{
	...
	case TCA6507_LED_RFB:
		...
		cmd[2] |= 0x04;
		cmd[3] |= 0x04;
		//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_led_blink]TCA6507_LED_RFB\r\n");
	...
	case TCA6507_LED_GFB:
		...
		//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_led_blink]TCA6507_LED_GFB\r\n");
		break;
	...
	case TCA6507_LED_KFB:
		...
		//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_led_blink]TCA6507_LED_KFB\r\n");
		break;
...
int tca6507_get_state(int led_selection)
{
	...
	for (i = JOGBALL_DISABLE; i > TCA6507_ABNORMAL_STATE; i--) {
		if (tca6507_drvdata.led_state[led_selection] & (1 << i)) {
			//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "%s: LED<%d> in <%d> State!!\n", __func__, led_selection, i);
...
static int tca6507_set_led(void)
{
	...
	else if(tca6507_drvdata.HWID >= CMCS_7627_EVB1)
	{
		cmd[2]=0x01;
		cmd_fade_off[2]=0x01;
		//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led]CMCS_7627_EVB1 	cmd_fade_off[2]=0x%x\r\n",cmd_fade_off[2]);	
	}
	dev_dbg(tca6507_dev, "%s\n", __func__);

	for (i = 0; i < tca6507_drvdata.LEDnum; i++) {
		state[i] = tca6507_get_state(i);
		//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led1]state[%d] = %d\r\n",i, state[i]);
		fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led]state[%d]=%d\r\n", i, state[i]); 

		switch (state[i]) {		
		case TCA6507_LED_OFF:
			if (TCA6507_LED_PREV_STATE_ON == tca6507_drvdata.prev_led_state[i]) {
				is_fade_off = true;
				//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led1]TCA6507_LED_OFF\r\n");
			}
			break;
		case TCA6507_LED_BLINK:
			//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led1]TCA6507_LED_BLINK\r\n");
		case CHARGER_STATE_LOW_POWER:
			is_blinking = true;
			cmd[10] = 0x4F;
			//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led1]CHARGER_STATE_LOW_POWER\r\n");
		...
		/*FIH, MichaelKao, 2009/07/07 {*/
			if (TCA6507_LED_PREV_STATE_OFF == tca6507_drvdata.prev_led_state[i]) {
				//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led1]TCA6507_LED_ON\r\n");
				//cmd[10] = 0xCF;
				is_fade_on = true;			
			}
	...
	for (i = 0; i < tca6507_drvdata.LEDnum; i++) {
		...
		case TCA6507_LED_OFF:
			if (TCA6507_LED_PREV_STATE_ON == tca6507_drvdata.prev_led_state[i]) {
				if (is_fade_off && is_fade_on) {
					cmd[10] = 0x4F;
					//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led2]TCA6507_LED_OFF i=%d\r\n",i);
					tca6507_led_fade_off(i, cmd_fade_off);
					tca6507_led_turn_off(i, cmd);	
				} else {
					//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led2]TCA6507_LED_OFF else\r\n");
					//tca6507_led_fade_off(i, cmd_fade_off);
					tca6507_led_fade_off(i, cmd);
				}
			} else {
				//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led2]TCA6507_LED_OFF turn off\r\n");
				tca6507_led_turn_off(i, cmd);			
			}
		...
		case CHARGER_STATE_LOW_POWER:
			...
			//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led2]CHARGER_STATE_LOW_POWER\n");
			break;
		case CHARGER_STATE_DISCHARGING:
		case TCA6507_LED_ON:
		case CHARGER_STATE_UNKNOWN:
		...
		/*FIH, MichaelKao, 2009/07/07 {*/
			if (TCA6507_LED_PREV_STATE_OFF == tca6507_drvdata.prev_led_state[i]) {
				...
				//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led2]TCA6507_LED_ON fade i=%d\r\n",i);
			} else {
				tca6507_led_turn_on(i, cmd);	
				
				//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_set_led2]TCA6507_LED_ON i=%d\r\n",i);
			}
	...
	if (is_fade_off && is_fade_on) {
		//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[TCA6507][tca6507_write]cmd_fade_off\n");
...
void tca6507_charger_state_report(int state) 
{
	...
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_charger_state_report]state=%d\r\n", state); 
	switch (state) {
	...
	case CHARGER_STATE_LOW_POWER2:		//RED BLINK
		//fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[TCA6507]CHARGER_STATE_LOW_POWER\r\n");
...
void tca6507_led_switch(bool on, int index)
{
	...
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_led_switch]index=%d on=%d\r\n", index, on); 
...
void tca6507_notify_blink(bool blink)
{
	...
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0,"[tca6507_notify_blink]blink = %d, tca6507_drvdata.is_suspend=%d\r\n",blink,tca6507_drvdata.is_suspend);
...
static ssize_t btn_brightness_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0,"[btn_brightness_store]brightness=0x%x\r\n",brightness);
...
static ssize_t Jogball_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_dbg(dev, "%s: %d\n", __func__, enable);
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0,"[btn_brightness_store]tca6507_Jogball_enable=0x%x\r\n",enable);
...
{
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "TCA6507 open\n" );
	if( ( file->f_flags & O_ACCMODE ) == O_WRONLY )
	{
		fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "TCA6507's device node is readonly\n" );
		return -1;
	}
	else
		return 0;
...
static int tca6507_miscdev_ioctl( struct inode * inode, struct file * filp, unsigned int cmd2, unsigned long arg )
{
  	int ret = 0;
	int index = 0;
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[TCA6507 ioctl] cmd2=%d\n",cmd2 );
    if(copy_from_user(&index, (int __user*)arg, sizeof(int)))
	{
		fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "Get user-space data error\n");
		return -1;
	}
	if(index < TCA6507_LED_RFB || index > TCA6507_LED_KFB3)
	{
	  	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[%s:%d]Wrong LED index\n", __func__, __LINE__);
	  	return -1;
	}
...
{
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_miscdev_release]\n");
	return 0;
}
				    ...
				    const struct i2c_device_id *id)
{
	struct tca6507_platform_data *tca6507_pd = client->dev.platform_data;
  	int i, ret = 0;
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "[tca6507_probe]\n");
...
static int __init tca6507_init(void)
{
	...
	/* FIH, Michael Kao, 2010/01/03{ */
	/* [FXX_CR], add for debug mask*/
	tca6507_debug_mask = 0xFFFFFF; //  *(int*)NLED_DEBUG_MASK_OFFSET;
	/* FIH, Michael Kao, 2010/01/03{ */

	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "Driver init: %s\n", __func__ );

	// i2c
	ret = i2c_add_driver(&tca6507_driver);
	if (ret) {
		fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "%s: Driver registration failed, module not inserted.\n", __func__);
		goto driver_del;
	}
...
/*Use miscdev*/
	...
	if (ret){
		fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "%s: Register misc device failed.\n", __func__);
		goto register_del;
	}
#endif
/*}FIH_FTM, PinyCHWu, 2009/06/15*/
...
{
	fih_printk(tca6507_debug_mask, FIH_DEBUG_ZONE_G0, "Driver exit: %s\n", __func__ );
/*FIH_FTM, PinyCHWu, 2009/06/15 {*/
/*new label name for remove misc device*/
#ifdef CONFIG_FIH_FXX
	misc_deregister(&tca6507_miscdev);
