...
#define AAT1272_DEBUG_MASK_VALUE *(unsigned int *)CAM_DEBUG_MASK_OFFSET
#define aat1272_user_g0(x...) \
	do { \
		if (AAT1272_DEBUG_MASK_VALUE & FIH_DEBUG_ZONE_G0) \
			fih_printk(AAT1272_DEBUG_MASK_VALUE, FIH_DEBUG_ZONE_G0, x); \
	} while (0)
#endif
/* } FIH, Charles Huang, 2009/12/28 */
...
static int aat1272_read( struct i2c_client *client, u8 *rxdata, int length )
{
	...
	{
		aat1272_user_g0(KERN_ERR "aat1272_read failed\n");
		return -EIO;
	}
...
static int aat1272_write( struct i2c_client *client, u8 *txdata, int length )
{
	...
	{
		aat1272_user_g0( KERN_ERR "aat1272_write failed\n" );
		return -EIO;
	}
...
static void flashled_enable(struct timed_output_dev *dev, int value)
{
	...
	aat1272_user_g0(KERN_INFO "%s: TIMER canceled\n", __func__);

	mutex_lock(&aat1272_drvdata.aat1272_lock);
	aat1272_user_g0(KERN_INFO "%s: flashled_timer period %d ms\n", __func__, value);
	...
	if (hrtimer_active(&flashled_timer))
		aat1272_user_g0(KERN_INFO "%s: TIMER running\n", __func__);
...
static int flashled_off_thread(void *arg)
{
	...
	aat1272_user_g0(KERN_INFO "%s: flashled_off_thread running\n", __func__);

	daemonize("flashled_off_thread");

	while (1) {
		wait_for_completion(&flashled_comp);
		aat1272_user_g0(KERN_INFO "%s: Got complete signal\n", __func__);
...
void brightness_onoff(int on)
{
	...
	aat1272_user_g0(KERN_INFO "flashdriver_store::on: %d\n", on);
	if (on==1){
...
void flash_settime(int time)
{
	...
	//mpp_22 = mpp_get(NULL, "mpp22");
	aat1272_user_g0(KERN_INFO "flashdriver_store::time: %d\n", time);
	if (time>0){
		ret=mpp_config_digital_out(mpp_22,MPP_CFG(MPP_DLOGIC_LVL_MSMP,
			MPP_DLOGIC_OUT_CTRL_HIGH));
		level = 16 - time;
...
static ssize_t brightness_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	aat1272_user_g0(KERN_INFO "flashdriver_store::enable: %d\n", enable);
	if (enable==1){
		brightness_onoff(1);
	}else{
		brightness_onoff(0);
...
static ssize_t flash_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	aat1272_user_g0(KERN_INFO "param1=%d,param2=%d\n",param1,param2);
				    ...
				    const struct i2c_device_id *id)
{
	...
	aat1272_user_g0(KERN_INFO "AAT1272 HW_ID: %d\n", aat1272_drvdata.HWID );
...
static int __init aat1272_init(void)
{
	...
	aat1272_user_g0(KERN_INFO "Driver init: %s\n", __func__ );

	ret = i2c_add_driver(&aat1272_driver);
	if (ret) {
		aat1272_user_g0(KERN_ERR "%s: Driver registration failed, module not inserted.\n", __func__);
		goto driver_del;
	}
...
{
	aat1272_user_g0(KERN_INFO "Driver exit: %s\n", __func__ );
	i2c_del_driver(&aat1272_driver);
}
