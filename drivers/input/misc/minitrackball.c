...
// Jogball depends on led driver to provide power control, if led driver isnot ready, set PORTING as 0 
#define PORTING 0
...
static uint32_t mtb_debug_mask = 0;
module_param_named(debug_mask, mtb_debug_mask, uint, 0644);

//static int lockjogball = -1; // 1: lock screen, 0: unlock screen
static int lockjogball = 0; // 1: lock screen, 0: unlock screen
...
#define SEND_JOGBALL_EVENT(EVENT, DELTA)	\
			do {		\
				if (time_after(jiffies, jogballResumeTime+HZ)) { \
					...
					input_sync(mtb_dev);		\
					fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "JogBall> %s (%d)\r\n", #EVENT, DELTA);	\
				}	\
				} \
				else { \
					fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "JogBall> ignore %s (%d)\r\n", #EVENT, DELTA);	\
				} \
			} while(0)
...
static int mtb_open(struct input_dev *dev)
{
	...
	fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "call mtb_open()\r\n");

	if(myJogBallData.jogball_opened == 1) {
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "jogball already open\r\n");
		return 0;
	}
...
{
	fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "call mtb_close()\r\n");

	if(myJogBallData.jogball_opened == 0) {
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "jogball already close\r\n");
		return;
	}
...
static void register_jogball_device(struct work_struct *work)
{
	...
	if (mtb_dev)
	{
		...
		if (err) {
			fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "register jogball device fail\r\n");
			input_free_device(mtb_dev);
		}		
		else {
			fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "register jogball device success\r\n");	
		}
	}
	else
	{
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "allocate jogball device fail\r\n");
	}
}
...
void mtb_early_suspend(struct early_suspend *h)
{
	if(myJogBallData.jogball_opened || !myJogBallData.jogball_exist) {
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "call mtb_early_suspend()\r\n");
...
{
	if(myJogBallData.jogball_opened || !myJogBallData.jogball_exist) {
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "call mtb_late_resume()\r\n");
...
static int __init mtb_platform_probe(struct platform_device *pdev)
{
	...
	fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "mtb_init() for HWID: %d, %d\r\n", myJogBallData.hwid, myJogBallData.oriHwid);
	...
	if(sysfs_create_group(&pdev->dev.kobj, &dev_attr_grp)) {
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "Create attributes in sysfs fail\r\n");
	}
	...
	if(myJogBallData.oriHwid >= CMCS_125_4G4G_FAA_PR1 &&  myJogBallData.oriHwid <= CMCS_128_4G4G_FAA_MP1)
	{
		myJogBallData.jogball_exist = -1;	
	}
	else if(myJogBallData.oriHwid >= CMCS_125_FST_PR1 &&  myJogBallData.oriHwid <= CMCS_128_FST_MP1)
	{
		myJogBallData.jogball_exist = -1;
	}
...
static int mtb_platform_suspend(struct platform_device *pdev, pm_message_t state)
{
	if(myJogBallData.jogball_opened) {
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "call mtb_platform_suspend()\r\n");
...
{
	if(myJogBallData.jogball_opened) {
		fih_printk(mtb_debug_mask, FIH_DEBUG_ZONE_G0, "call mtb_platform_resume()\r\n");
...
static __init int mtb_init(void)
{
	...
	mtb_debug_mask = *(uint32_t *)MTB_DEBUG_MASK_OFFSET;
