...
static uint32_t fihsensor_debug_mask = 0;
module_param_named(debug_mask, fihsensor_debug_mask, uint, 0644);
...
static int compass_read_reg(struct i2c_client *clnt, unsigned char reg, unsigned char *data, unsigned char count)
{
	...
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_read_reg() set REGISTER address error\r\n");
		return -EIO;
	}

	if(count != i2c_master_recv(clnt, tmp, count))
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_read_reg() read REGISTER content error\r\n");
		return -EIO;
	}
...
static int compass_write_reg(struct i2c_client *clnt, unsigned char reg, unsigned char *data, unsigned char count)
{
	...
	while(count)
	{
		...
		{
			fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_write_reg() error\r\n");
			return -EIO;
		}
...
static int enter_mode(unsigned char mode)
{
	...
	if(mode==SMB380_MODE_NORMAL || mode==SMB380_MODE_SLEEP) 
	{
		...
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "gsensor: WAKE_UP_REG (0x%x)\r\n", data1);
		data1 = SMB380_SET_BITSLICE(data1, WAKE_UP, mode);

		if((ret=compass_read_reg(smb380_i2c_client, SLEEP_REG, &data2, 1)))
			return ret;
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "gsensor: SLEEP_REG (0x%x)\r\n", data2);		
...
{
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass: start_suspend\r\n");
...
{
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass: start_resume\r\n");
...
{
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_open()\r\n");

	fihsensor_ctx.depth++;

	if(fihsensor_ctx.depth > 1)
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "warning: compass reopen\r\n");
		return 0; 
	}
...
/* implement suspend/resume */
	...
	{
		if(start_resume()) {
			fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "fihsensor: start_resume() fail\r\n");
			fihsensor_ctx.depth--;
			return -EIO;
		}
		else {
...
{
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_release()\r\n");
...
/* implement suspend/resume */
	...
	{
		if(start_suspend()) {
			fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "fihsensor: start_suspend() fail\r\n");
		}
		else {
	   ...
	   unsigned long arg)
{
	...
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_ioctl(): cmd (0x%X)\r\n", cmd);

	switch (cmd) {
	case IOCTL_SET_ACTIVE_SLAVE:
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_ioctl(): set active slave (0x%X)\r\n", (unsigned int)arg);
		
		switch (arg) {
		case SMB380_I2C_ADDR:
			fihsensor_ctx.activeSlave = smb380_i2c_client;
			fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_ioctl() active slave=0x%X (gsensor)\r\n", (unsigned int)fihsensor_ctx.activeSlave);
		...
		case MS3CDRV_I2C_SLAVE_ADDRESS:
			fihsensor_ctx.activeSlave = ms3c_i2c_client;
			fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_ioctl() active slave=0x%X (ecompass)\r\n", (unsigned int)fihsensor_ctx.activeSlave);
                            ...
                            size_t count, loff_t *pos)
{
	...
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_read()\r\n");

/* FIH; Tiger; 2009/11/30 { */
/* restrict others to access compass */
	if(fp->private_data != (void*)current) {
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "don't allow compass_read()\r\n");		
		return -EPERM;
	}
/* } FIH; Tiger; 2009/11/30 */
...
/* implement suspend/resume */
	...
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_read() in idle mode\r\n");
		return -EIO;
	}
#endif
/* } FIH; Tiger; 2009/6/10 */	
	...
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_read: device invalid\n");
		ret = -ENODEV;	
	}
	else 
	{
		...
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_read result: (%d, %d, %d)\n", 
				(fihsensor_ctx.activeSlave == ms3c_i2c_client), 
				count, ret);

#if 1
		if(fihsensor_debug_mask&FIH_DEBUG_ZONE_G1) 
		{
			int i;
			for(i=0; i< count; i++) 
			{
				fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_read() data [%d, 0x%X]\r\n", i, tmp[i]); 
			}
		}
#endif
	}	
                             ...
                             size_t count, loff_t *pos)
{
	...
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_write()\r\n");
	
/* FIH; Tiger; 2009/11/30 { */
/* restrict others to access compass */
	if(fp->private_data != (void*)current) {
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "don't allow compass_write()\r\n");
		return -EPERM;
	}
/* } FIH; Tiger; 2009/11/30 */	
...
/* implement suspend/resume */
	...
	if(fihsensor_ctx.bIsIdle == true && fihsensor_ctx.activeSlave == smb380_i2c_client)
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_write() in idle mode\r\n");
		return -EIO;
	}
#endif
/* } FIH; Tiger; 2009/6/10 */
	...
	if(fihsensor_ctx.activeSlave == NULL) 
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "compass_write: device invalid\n");
		ret = -ENODEV;	
	}
	else 
	{
		...
		else
		{
			...
			if(fihsensor_debug_mask&FIH_DEBUG_ZONE_G1)
			{
				...
				for(i=0; i< count; i++) 
				{
					fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_write() data [%d, 0x%X]\r\n", i, tmp[i]); 
				}
			}
#endif
		...
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_write() active slave=0x%X\r\n", (unsigned int)fihsensor_ctx.activeSlave);

		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G1, "compass_write result: (%d, %d, %d)\n", 
				(fihsensor_ctx.activeSlave == ms3c_i2c_client), 
				count, ret);
	}
...
{
	int err;
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "ms3c_probe()\r\n");
	...
	if (err) {
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "ms3c_probe: compass_device register failed\n");
...
//exit_check_functionality_failed:
	...
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "ms3c_probe() fail\r\n");
	return err;
}
...
{
	int err;
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "smb380_probe()\r\n");
...
//exit_check_functionality_failed:
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "smb380_probe() fail\r\n");
	return err;
}
...
/* implement suspend/resume */
#if FEATURE_SLEEP
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "smb380_suspend()\r\n");
...
/* implement suspend/resume */
#if FEATURE_SLEEP
	fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "smb380_resume()\r\n");
...
static int __init fihsensor_init(void)
{
	...
	{
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "YAMAHA MS-3C compass driver init fail\r\n");	
	}
	else if((ret=i2c_add_driver(&smb380_driver)))
	{
		i2c_del_driver(&ms3c_driver);
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "SMB380 driver init fail\r\n");	
	}
	else
	{
	
		fih_printk(fihsensor_debug_mask, FIH_DEBUG_ZONE_G0, "FIH sensor driver init success\r\n");	
	}
