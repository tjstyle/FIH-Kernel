...
#include <linux/pm.h>  //Added for new kernel suspend scheme (2010.08.25)
...
static void bi8232_isr_workqueue(struct work_struct *work)
{
	...
	if (buffer[0] == 0x5A) {
        ...
        if((virtual_button == 0) && !bSoft1CapKeyPressed && !bSoft2CapKeyPressed && !bHomeCapKeyPressed && !bApCapKeyPressed && !bCenterKeyPressed && !bIsNeedSkipTouchEvent)  /* FIH, Henry Juang, 2009/11/20 ++*/ /* [FXX_CR], Add for proximity driver to turn on/off BL and TP. */
        {
			...
			//Added the MT protocol for Eclair by Stanley (2010/03/23)--
            //Added log for debugging by Stanley (2010.08.19)++
            //if (msm_cap_touch_debug_mask & MSM_PM_DEBUG_COORDINATES)  //Added for debug mask definitions
            //{
                if (cnt == 1 && bPrintPenDown)
                {
                    //bi8232_msg(INFO, "[TOUCH-CAP]Pen down x = %d, y = %d", 1792 - XCORD1(buffer), YCORD1(buffer));
                    fih_printk(msm_cap_touch_debug_mask, FIH_DEBUG_ZONE_G1, "[TOUCH-CAP]Pen down x = %d, y = %d\r\n", 1792 - XCORD1(buffer), YCORD1(buffer));  //Added for debug mask (2010/01/04)
                    bPrintPenDown = 0;
                }
                if (cnt == 2)
                {
                    //bi8232_msg(INFO, "[TOUCH-CAP]Pen down x = %d, y = %d", 1792 - XCORD1(buffer), YCORD1(buffer));
                    fih_printk(msm_cap_touch_debug_mask, FIH_DEBUG_ZONE_G1, "[TOUCH-CAP]Pen down x2 = %d, y2 = %d\r\n", 1792 - XCORD2(buffer), YCORD2(buffer));
                    bPrintPenDown = 0;
                }
                else if (!cnt)
                {
                    //bi8232_msg(INFO, "[TOUCH-CAP]Pen up x = %d, y = %d", XCORD1(buffer), YCORD1(buffer));
                    fih_printk(msm_cap_touch_debug_mask, FIH_DEBUG_ZONE_G1, "[TOUCH-CAP]Pen up x = %d, y = %d\r\n", XCORD1(buffer), YCORD1(buffer));  //Added for debug mask (2010/01/04)
                    bPrintPenDown = 1;
                }
            //}
            //Added log for debugging by Stanley (2010.08.19)++--
        ...
        else if ((virtual_button == 0) && (bSoft1CapKeyPressed || bSoft2CapKeyPressed || bHomeCapKeyPressed || bApCapKeyPressed || bCenterKeyPressed) && !bIsKeyLock)  //Button up  //Modified for Home and AP key (2009/07/31)
        {
            ...
            else if(bApCapKeyPressed)
            {
                ...
                //else if(bIsGRECO)
                else  //Added for Froyo by Stanley (2010/09/28)
...
static int bi8232_suspend(struct device *client)  //Added for new kernel suspend scheme (2010.08.25)
{
    ...
    struct elan_i2c_platform_data *pdata = bi8232->client->dev.platform_data;  //Added for new kernel suspend scheme (2010.08.25)
    int ret, ret_gpio = 0;  //Add for VREG_WLAN power in, 07/08

	cancel_work_sync(&bi8232->wqueue);
    printk(KERN_INFO "bi8232_suspend() disable IRQ: %d\n", bi8232->client->irq);  //Added for new kernel suspend scheme (2010.08.25)
    disable_irq(bi8232->client->irq);  //Added for new kernel suspend scheme (2010.08.25)
...
static int bi8232_resume(struct device *client)  //Added for new kernel suspend scheme (2010.08.25)
{
    struct vreg *vreg_wlan;  //Add for VREG_WLAN power in, 07/08
...
//Remove to set power state by Stanley--
	...
	enable_irq(bi8232->client->irq);  //Added for new kernel suspend scheme (2010.08.25)
	//Modify the scheme for receive hello packet--
	return 0;
}
#else
...
static int bi8232_fst_suspend(struct device *client)  //Added for new kernel suspend scheme (2010.08.25)
{
    struct vreg *vreg_wlan;  //Add for VREG_WLAN power in, 07/08
    //struct elan_i2c_platform_data *pdata = client->dev.platform_data;  //Setting the configuration of GPIO 89
    struct elan_i2c_platform_data *pdata = bi8232->client->dev.platform_data;  //Added for new kernel suspend scheme (2010.08.25)
    ...
    {
        disable_irq(bi8232->client->irq);  //Added for new kernel suspend scheme (2010.08.25)
	    if (device_may_wakeup(&bi8232->client->dev)) {  //Added for new kernel suspend scheme (2010.08.25)
	        enable_irq_wake(bi8232->client->irq);  //Added for new kernel suspend scheme (2010.08.25)
...
static int bi8232_fst_resume(struct device *client)  //Added for new kernel suspend scheme (2010.08.25)
{
    struct vreg *vreg_wlan;  //Add for VREG_WLAN power in, 07/08
    //struct elan_i2c_sensitivity sen;  //Added for modify sensitivity, 0729
    //struct elan_i2c_platform_data *pdata = client->dev.platform_data;  //Setting the configuration of GPIO 89
	...
	else
	{
        enable_irq(bi8232->client->irq);  //Added for new kernel suspend scheme (2010.08.25)
	    if (device_may_wakeup(&bi8232->client->dev)) {  //Added for new kernel suspend scheme (2010.08.25)
	        disable_irq_wake(bi8232->client->irq);  //Added for new kernel suspend scheme (2010.08.25)
...
static int __init bi8232_init( void )
{
    ...
    {
		 msm_cap_touch_debug_mask = *(uint32_t *)TOUCH_DEBUG_MASK_OFFSET;
         //Neo: Add for increasing VREG_WLAN refcnt and let VREG_WLAN can be closed at first suspend +++
         #if 0
         if((FIH_READ_HWID_FROM_SMEM() != CMCS_7627_PR1) && (FIH_READ_HWID_FROM_SMEM() != CMCS_F913_PR1))  //Don't apply VREG_WLAN power in on PR1++
         {
