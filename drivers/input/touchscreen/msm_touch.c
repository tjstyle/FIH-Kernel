...
static uint32_t msm_res_touch_debug_mask;
module_param_named(
    debug_mask, msm_res_touch_debug_mask, uint, S_IRUGO | S_IWUSR | S_IWGRP
);
//Added for debug mask (2010/01/04)--
...
static irqreturn_t ts_interrupt(int irq, void *dev_id)
{
	...
	if ((num_op == TSSC_NUMBER_OF_OPERATIONS) && (num_samp == 0)) {
        ...
        fih_printk(msm_res_touch_debug_mask, FIH_DEBUG_ZONE_G1, "[TOUCH] Interrupt : {%3d, %3d},"
				" op = %3d samp = %3d si = %d\r\n", lx, ly, num_op, num_samp, si);  //Added for debug mask (2010/01/04)
#endif
...
/* } FIH, SimonSSChang, 2009/09/04 */
	...
	gpio_tlmm_config(GPIO_CFG(89, 0, GPIO_INPUT,
...
static int __init ts_init(void)
{
    ...
    {
        msm_res_touch_debug_mask = *(uint32_t *)TOUCH_DEBUG_MASK_OFFSET;  //Added for debug mask (2010/01/04)
