 ...
 * */
int HWID = 0;
int ORIGHWID = 0; ////FIH, Nicoel, 2010/07/20
int cm3602_pid =0;
...
static int cm3602_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg){
	...
	fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "cm3602_ioctl,cmd=%d,arg=%ld\n" ,cmd,arg);
	...
	switch (cmd) {
		case CM3602_PS_OFF: //For CM3602 proximity sensor switch off.
			...
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_PS_OFF setting GPIO %d = Pull High\n", CM3602_PS_GPIO );	
		...
		case CM3602_PS_ON: //For CM3602 proximity sensor switch on.
			...
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_PS_ON setting GPIO %d,GPIO %d = Pull Low\n", CM3602_EN_GPIO,CM3602_PS_GPIO );
			
			if(flag_cm3602_irq==0)
			{
				enable_irq(cm3602_irq);	
				flag_cm3602_irq=1;
				fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: enable_irq cm3602_irq=%d\n", cm3602_irq );
			}
#endif
			return 0;		
		case CM3602_ALS_ON: //For CM3602 light sensor switch on.		
			enableALS = 1;	
			gpio_direction_output(CM3602_EN_GPIO,0);
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_ALS_ON setting GPIO %d,GPIO %d = Pull Low\n", CM3602_EN_GPIO,CM3602_PS_GPIO );
			return 0;			
		case CM3602_ALS_OFF: //For CM3602 light sensor switch off.		
			...
			gpio_direction_output(CM3602_PS_GPIO,1);
#endif
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_ALS_OFF setting GPIO %d , %d= Pull High\n", CM3602_EN_GPIO,CM3602_PS_GPIO );
			return 0;
			break;
		case CM3602_ALS_READ:   //Read Light Sensor Value after ADC
			...
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_ALS_READ = %d\n", value );
			return value;
		...
		case CM3602_PS_ON_SIGNAL:			
#ifndef DISABLE_PROXIMITY 
			{
				...
				{
					fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: ioctl pid = %d\n", tsk->pid );
		...
		case CM3602_PS_READ:
			...
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_PS_READ = %d\n", value );
			return value;
#endif
			break;
		case CM3602_ACTIVATE:
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_ACTIVATE = %ld\n", arg );
			g_activate = arg;
			return 1;
		case CM3602_Proximity_Status:
#ifndef DISABLE_PROXIMITY 
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: HWID = %d\n", HWID );
			if ((HWID >= CMCS_RTP_PR2) && ((HWID <= CMCS_RTP_MP3)))
				return 0;
			else
				return 1;
...
/* [FXX_CR], Add for proximity driver to turn on/off BL and TP. */
		case CM3602_FQC_Testing:
			isFQC_Testing=arg;
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: isFQC_Testing = %d\n", isFQC_Testing );
			return 0;
/* FIH, Henry Juang, 2009/11/20 --*/
		case CM3602_OFF: //For CM3602 whole switch off.			
			...
			gpio_direction_output(CM3602_PS_GPIO,1);
			#endif
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: CM3602_OFF setting GPIO %d , %d= Pull High\n", CM3602_EN_GPIO,CM3602_PS_GPIO );
return 0;		
		default:
			return -EINVAL;
	}
...
/* implement fast path for ALS/PS */	
	...
	/* [FXX_CR], Add for proximity driver to turn on/off BL and TP. */
	if(!isFQC_Testing){
		fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "cm3602_trigger\n");
...
{	
	...
	if(enableALS) {
		...
		//FIH, Nicoel, 2010/07/20
		//if (HWID >= CMCS_CTP_PR1 && HWID!=CMCS_FM6_PR1)
		if (HWID >= CMCS_CTP_PR1 && !(ORIGHWID>= CMCS_128_FM6_PR1 && ORIGHWID <= CMCS_125_FM6_MP)&& !(ORIGHWID>= CMCS_FN6_PR1 && ORIGHWID <= CMCS_FN6_MP1))
			st[0] = st[0] * 23/100;		
	}
	else {
		st[0] = (char)-1;		
	...
	fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: PS level = %d\n", st[1]);
	fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: ALS level = %d\n", st[0]);
...
{
	fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "cm3602_dev_open\n");
	return 0;
}
...
static int __init cm3602_init(void)
{
	...
	*/
	...
	fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0,"CM3602 platform_driver_register=%d\n",ret);
	...
	fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0,"CM3602 HWID=%d\n",HWID);
	if (!((HWID >= CMCS_RTP_PR2) && ((HWID <= CMCS_RTP_MP3)))){
	if (HWID >= CMCS_CTP_PR1)
		fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0,"ALS Table weighting: 0.23\n");
		
	rc = gpio_tlmm_config(GPIO_CFG(CM3602_EN_GPIO, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
	if (rc) {
		fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0,
...
/* FIH, Henry Juang, 2009/11/20 --*/
	fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0, "CM3602: request_irq = %d %d\n", ret, cm3602_irq );
...
//FIH, HenryJuang 2009/11/11 --
	...
	rc = gpio_tlmm_config(GPIO_CFG(CM3602_PS_GPIO, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
	if (rc) {
		fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0,
				"%s--%d: gpio_tlmm_config=%d\n",
				__func__,__LINE__, rc);
	...
	if (HWID == CMCS_HW_VER_EVB1)
	{
		rc = gpio_tlmm_config(GPIO_CFG(CM3602_EVB1_PS_GPIO_OUT, 0, GPIO_INPUT, GPIO_PULL_DOWN/*GPIO_NO_PULL*/, GPIO_2MA), GPIO_ENABLE);
		if (rc) {
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0,
					"%s--%d: gpio_tlmm_config=%d\n",
					__func__,__LINE__, rc);
	...
	{
		rc = gpio_tlmm_config(GPIO_CFG(CM3602_PR1_PS_GPIO_OUT, 0, GPIO_INPUT, GPIO_PULL_DOWN/*GPIO_NO_PULL*/, GPIO_2MA), GPIO_ENABLE);
		if (rc) {
			fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0,
					"%s--%d: gpio_tlmm_config=%d\n",
					__func__,__LINE__, rc);
	...
	if (rc){
		fih_printk(cm3602_debug_mask, FIH_DEBUG_ZONE_G0 ,"%s: msm alsps setting failed! rc = %d\n", __func__, rc);
		return -EIO;
	}
	gpio_direction_output(CM3602_PS_GPIO, 0);
