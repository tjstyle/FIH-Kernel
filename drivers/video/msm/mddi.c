...
static int mddi_resume(struct platform_device *pdev)
{
	...
	//clk_enable(mddi_clk);
	//chandler_failon
	if(clk_enable(mddi_clk)){
        fih_printk(debug_mask_lcm, FIH_LCM_ERR,KERN_DEBUG"clk_enable(mddi_clk) fail!!\n");
	}else{
        fih_printk(debug_mask_lcm, FIH_LCM_DBG,KERN_DEBUG"clk_enable(mddi_clk) succeed!!\n");
	}
    /* FIH, ChandlerKang ,09.12.14 } */
	if (mddi_pclk)
