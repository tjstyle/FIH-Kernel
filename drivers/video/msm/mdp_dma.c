...
void mdp_dma_pan_update(struct fb_info *info)
{
    ...
    ktime_t now_time;
    int64_t diff=0;
    /* For check transfer time */
    static struct timespec transfer_time={0,0};
    ktime_t before_now_time;
    static uint32 update_count=0;
    update_count++;
    before_now_time=ktime_get();
#endif
    ...
    diff=ktime_to_ns(ktime_sub(now_time,last_time));    
    fps_count++;
    transfer_time=timespec_add_safe(transfer_time, ktime_to_timespec( ktime_sub(now_time,before_now_time)));
    //printk(KERN_INFO " fps_count: %d\n", fps_count );
    if(diff > NSEC_PER_SEC){
        struct timespec tmp=ktime_to_timespec( ktime_sub(now_time,last_time));
        unsigned long fps=fps_count*1000000/(tmp.tv_sec*1000000+tmp.tv_nsec/1000);
        if(fps){
            printk(KERN_DEBUG " fps:  %lu ",fps);
            printk(KERN_DEBUG " trnsfer_time(us): %lu \n",(transfer_time.tv_sec*1000000+transfer_time.tv_nsec/1000)/fps);
        }
        fps_count=0;
        last_time=now_time;
        transfer_time.tv_nsec=0;
        transfer_time.tv_sec=0;        
    }
#endif		
}
