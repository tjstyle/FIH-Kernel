...
do { \
    ...
    if(ret) fih_printk(debug_mask_lcm, FIH_LCM_ERR, KERN_ERR "[lcm_innolux] SPI write error  0x%x.\n", data);\
} while(0)
...
void reset_lcmic(void)
{
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: reset_lcmic(): gpio_direction_output hight: read(%d)\n", gpio_get_value(gpio_lcd_reset));
    mdelay(1);

    ret = gpio_direction_output(gpio_lcd_reset, 0);
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: reset_lcmic(): gpio_direction_output low: read(%d)\n", gpio_get_value(gpio_lcd_reset));
    mdelay(10);
...
int ilitek_config(struct driver_data *dd, struct spi_device *spi)
{
    int ret=0;
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: ilitek_config()\n");
...
{
    int ret=0;
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: innolux_config()\n");
    //reset_lcmic();
...
int innolux_config_v3(struct driver_data *dd, struct spi_device *spi)
{
    ...
    int ret=0;
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: innolux_config_v3()\n");
...
int simple_config(struct driver_data *dd, struct spi_device *spi)
{
    int ret=0;
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: simple_config()\n");
...
int get_power_mode(struct driver_data *dd, struct spi_device *spi)
{
    ...
    int ret=0;
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: get_power_mode()\n");
    LCD_ILI9481_CMD(0x11);    
    Delayms(20);
...
*/
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: get_pixel_mode(), tx(0x%x), rxbuf[]=[0x%x,0x%x]\n", txbuf, rxbuf[0], rxbuf[1] );
...
int sleep_config(struct driver_data *dd, struct spi_device *spi)
{
    ...
    reset_lcmic();
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: get_pixel_mode()\n");
...
inline int fih_lcdc_config(struct driver_data *dd, struct spi_device *spi)
{
    ...
    int ret=0;
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: fih_lcdc_config()\n");    
...
void fih_lcdc_enter_sleep(void)
{
    ...
    if (!spi){
        fih_printk(debug_mask_lcm, FIH_LCM_ERR, KERN_INFO "lcm_innolux: fih_lcdc_enter_sleep(), spi NULL! do nothing.\n");    
        return;
    }

    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: fih_lcdc_enter_sleep()\n");    
...
void fih_lcdc_exit_sleep(void)
{
    ...
    if (!spi){
        fih_printk(debug_mask_lcm, FIH_LCM_ERR, KERN_INFO "lcm_innolux: fih_lcdc_exit_sleep(), spi NULL! do nothing.\n");    
        return;
    }

    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: fih_lcdc_exit_sleep()\n");    
...
static int lcdc_spi_probe(struct spi_device *spi)
{
	...
	fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux:%s(): \n", __func__);
...
static int __init lcdc_innolux_init(void)
{
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: +lcdc_innolux_init()\n");
    
    if(fih_lcm_is_mddi_type()){
        fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: lcdc_innolux_init(): LCM isn't RGB i/f\n");
        lcdc_clk_off();        
        ret=-ENODEV;
        goto err;
    }
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_innolux: +lcdc_innolux_init(): pinfo.clk_rate(%d)\n", pinfo.clk_rate);
