...
static int wintek_lcd_on(struct platform_device *pdev)
{
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: wintek_lcd_on():panel_first_on(%d)\n", panel_first_on);
...
/* FIH, ChandlerKang, 10.2.10, for innolux mddi panel } */
        ...
        if(mddi_write_ret==-1 && !retry){
            fih_printk(debug_mask_lcm, FIH_LCM_ERR, KERN_ERR"[lcm_wintek] unknow failure, try to re-init mddihost \n");
            mddi_write_ret=0;
    	    mddi_host_reinit();
    	    retry=1;
            goto RETRY;
...
static int wintek_lcd_off(struct platform_device *pdev)
{
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: wintek_lcd_off()\n");
...
void panel_off_wintek(void)
{
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: mddi_queue_shutdown_write()!!\n");
}
...
static void reset_mddi_lcm(void)
{
    ...
    */
    ...
    if( FIH_READ_HWID_FROM_SMEM() >= CMCS_7627_PR1 ){
    	...
    	fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: %s(): gpio(112) gpio_read = %d\n",__func__, gpio_get_value(112));
    }

    /* LCD reset pin: pull high */
    ret = gpio_direction_output(gpio_lcd_reset, 0);
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: %s(): gpio_get_value(%d) low: rc(%d)\n",__func__,gpio_get_value(gpio_lcd_reset),ret);
    mdelay(10);
    ...
    if( FIH_READ_HWID_FROM_SMEM() == CMCS_CTP_PR1 ){
        ...
        fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: %s(): gpio_direction_output_a(%d) low: rc(%d)\n",__func__, gpio_lcd_reset_a, ret);
        mdelay(10);
        
        ret = gpio_direction_output(gpio_lcd_reset_a, 1);
        fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: %s(): gpio_direction_output_a(%d) hight: rc(%d)\n",__func__, gpio_lcd_reset_a, ret);
        mdelay(10);
    }
...
void panel_on_inl_30pins(void)
{
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG,KERN_INFO "panel_on_inl_30-10.01.21: %s()\n",__func__);    
...
void panel_on_wintek_30pins(void)
{
    ...
    */
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: %s()\n",__func__);
...
{
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: wintek_probe()\n");

    if(!fih_lcm_is_mddi_type()){
        fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: wintek_probe(): LCM isn't MDDI i/f\n");
        return -ENODEV;
    }
...
void get_otp_id(uint32 id_addr,uint32 *ptr_value)
{
    ...
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO"%s(): id_addr 0x%x: value:%d\n",__func__,id_addr,*ptr_value);
#else
    mddi_queue_register_read(0x60+id_addr,ptr_value,TRUE,0);  
    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO"%s(): reg 0x%x: value:%d\n",__func__,0x60+id_addr,*ptr_value);
#endif
}
...
void check_panel_id(void)
{
    ...
    if(otp_id1 == 0x01 && otp_id2 == 0x01 && otp_id3 == 0x01 ){
        fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO"%s(): found inl panel\n",__func__);
...
//#ifdef CONFIG_FB_MSM_MDDI_AUTO_DETECT
    ...
    if( (hwid >= CMCS_CTP_PR1 && hwid < CMCS_7627_EVB1 ) || (hwid >= CMCS_7627_PR1 ) ) {
	    ...
	    fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: wintek_init(): Force LCM as MDDI type, hwid(%d)\n",hwid);
    	lcm_is_mddi_type = 1;
        ...
        if(id) {
            lcm_is_mddi_type=1;
            fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: wintek_init(): the LCM is MDDI type, id(%d)!!\n",id);	
        } else {
            lcm_is_mddi_type=0;
            fih_printk(debug_mask_lcm, FIH_LCM_DBG, KERN_INFO "lcm_wintek: wintek_init(): no MDDI LCM found!!, hwid(%d)\n", hwid);	
        }
    }
    /* Chandler, 2009/06/08 } */
