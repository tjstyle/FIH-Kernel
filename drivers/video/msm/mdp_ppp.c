...
#if defined (CONFIG_FIH_PROJECT_FM6) || defined (CONFIG_FIH_PROJECT_FN6)
...
#if 0
	printk(KERN_ERR "%s(): src(%d,%d,%d,%d),rect(%d,%d,%d,%d),dst(%d,%d,%d,%d),rect(%d,%d,%d,%d),s(%lu),a(%u),f(%u)\n", __func__, 
		req->src.width, req->src.height, req->src.format, req->src.offset,
		req->src_rect.x, req->src_rect.y, req->src_rect.w, req->src_rect.h,
		req->dst.width,req->dst.height, req->dst.format, req->dst.offset,
		req->dst_rect.x, req->dst_rect.y, req->dst_rect.w, req->dst_rect.h,dst_start,req->alpha,req->flags);
//#else
	printk(KERN_ERR "%s():  src rect:(%d,%d,%d,%d), dst rect:(%d,%d,%d,%d)...\n", __func__, 
...
/* workaround for the performance of a game*/
    ...
    if( (req->src.format == MDP_RGBA_8888) && (req->flags & MDP_ROT_90) && req->alpha ==255 &&
        (req->src_rect.w==req->src.width && req->src_rect.h==5 && req->dst_rect.x== 0 && req->dst_rect.y==0) ){
...
/* } FIH, ChandlerKang, 09/11/25 */
	...
	if (((iBuf.mdpImg.mdpOp & (MDPOP_TRANSP | MDPOP_ALPHAB)) ||
		...
		/* FIH, IvenHuang, 2010/11/12 {*/
		int reverse_mode = 0;
		if ( mdpOp & ( MDPOP_LR | MDPOP_UD )) {
			reverse_mode = 1;
			iBuf.roi.lcd_y += ( req->dst_rect.h - 16 );
		}
		/* } FIH, IvenHuang, 2010/11/12 */
/*		
		printk(KERN_ERR "[FM6]MDP_PPP BLIT iBuf.mdpImg.mdpOp=%d, req->src.format=%d, req->alpha=%d, req->dst_rect.w=%d!!!!!\n",iBuf.mdpImg.mdpOp, req->src.format, req->alpha, req->dst_rect.w);
        printk(KERN_ERR "[FM6] mdpOp(0x%x), src.format(0x%x)\n alpha(0x%x),trans(0x%x),x(%d),y(%d),w(%d),h(%d)  x(%d),y(%d),w(%d),h(%d)\n",
            iBuf.mdpImg.mdpOp,req->src.format,req->alpha,req->transp_mask,
            req->src_rect.x,req->src_rect.y,
            req->src_rect.w,req->src_rect.h,
            req->dst_rect.x,req->dst_rect.y,
        ...
        printk(KERN_ERR "[FM6] src:offset(%d),format(%d),w(%d),h(%d)  dst:offset(%d),format(%d),w(%d),h(%d)\n",
            req->src.offset,req->src.format,
            req->src.width,req->src.height,
            req->dst.offset,req->dst.format,
            req->dst.width,req->dst.height);
*/	
		...
		for (i = 0; i < (req->dst_rect.h / 16); i++) {
			...
			if (((MDP_SCALE_Q_FACTOR * iBuf.roi.dst_height) /
				...
				iBuf.roi.width =
				    ...
				    MDP_MIN_X_SCALE_FACTOR;
#if 0
			printk(KERN_ERR "%s():  roi:(%d,%d)(%d,%d)\n", __func__, 
			iBuf.roi.width, iBuf.roi.height, iBuf.roi.dst_width, iBuf.roi.dst_height);
#endif
			mdp_start_ppp(mfd, &iBuf, req, p_src_file, p_dst_file);

			/* next tile location */
			iBuf.roi.lcd_y += (reverse_mode?-16:16); /* FIH, IvenHuang, 2010/11/12 */
		...
		if ((dst_h > 0) && (src_w > 0)) {
			...
			if(iBuf.roi.width<=16) {
				if ( reverse_mode ) iBuf.roi.lcd_y += (16 - iBuf.roi.width); /* FIH, IvenHuang, 2010/11/12 */
				mdp_start_ppp(mfd, &iBuf, req, p_src_file, p_dst_file);
			} else{
...
/* workaround for the performance of a game*/
    ...
    if( (req->src.format == MDP_RGBA_8888) && (req->flags == MDP_ROT_90) && req->alpha ==255 &&
        ...
        int i,j;
        uint32 r,g,b,alpha,alpha_re;
		byte *rgba;
		int16 *p;
		uint32 *p_src_img=(uint32*)ioremap(src_start+req->src.offset, src_len);
		uint16 *p_dst_img=(uint16*)ioremap(dst_start+req->dst.offset, dst_len);
...
#if 0
				rgba=(byte*)(p_src_img+((j+req->src_rect.y)*480)+(i+req->src_rect.x));          
                p=p_dst_img+ ((i+req->dst_rect.y)*320)+ (req->dst_rect.x+(req->dst_rect.w-1-j));

				alpha=rgba[3];
				alpha_re=255-alpha;
				r=((rgba[0])*alpha)+(((((*p)&0xf800)>>11)<<3)*alpha_re);
				g=((rgba[1])*alpha)+(((((*p)&0x07E0)>>5 )<<2)*alpha_re);
				b=((rgba[2])*alpha)+(((((*p)&0x001F)    )<<3)*alpha_re);
