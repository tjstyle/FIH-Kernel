...
{
/* FIH, ChandlerKang, 09.12.14 { */
	//static boolean initialized = FALSE;// chandler_failon
/* FIH, ChandlerKang, 09.12.14 } */
	mddi_host_cntl_type *pmhctl;
...
uint16 mddi_get_next_free_llist_item(mddi_host_type host_idx, boolean wait)
{
    ...
    //printk(KERN_INFO"%s():chandler: host_idx(%d),wait(%d).\n",__func__,ret_idx,wait);
	pmhctl->llist_info.next_free_idx++;
...
/* FIH, ChandlerKang, 09.12.14 { */
	    //chandler_failon
        printk(KERN_INFO"%s():chandler: llist_notify[ret_idx].in_use, wait(%d)\n",__func__,wait);
/* FIH, ChandlerKang, 09.12.14 } */
		if (!wait) {
	...
	if (forced_wait) {
	    printk(KERN_INFO"%s():chandler: forced_wait true.\n",__func__);
