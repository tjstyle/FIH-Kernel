...
/* FIH, ChandlerKang, 2010.8.27 tethering with diag+++ */
static int tethering_diag=0;
static int android_set_tethering_diag(const char *val, struct kernel_param *kp);
static int android_get_tethering_diag(char *buffer, struct kernel_param *kp);
module_param_call(tethering_diag, android_set_tethering_diag, android_get_tethering_diag,
					&tethering_diag, 0664);
MODULE_PARM_DESC(tethering_diag, "Enable/Disable tethering with diag");
...
{
	int ret;
	ret = sprintf(buffer, "%x", tethering_diag);
...
/* FIH, ChandlerKang, 2010.8.27 tethering with diag +++ */
	if(tmp == 0xc008 && tethering_diag){
		printk(KERN_INFO "change product id to 0xc007, (%d)\n",tethering_diag);
		tmp = 0xc007; // for modem diag while usb tethering enable
	}
/* FIH, ChandlerKang, 2010.8.27 tethering with diag --- */
...
static int adb_enable_open(struct inode *ip, struct file *fp)
{
	...
	//if (dev->adb_enabled) //unneccesary; marked by WilsonWHLee 
		goto out;
...
///WilsonWHLee 2010/02/23 5110 porting ------------
	...
	strlcpy(serial_number , pdata->serial_number , strlen(pdata->serial_number)); //porting 6030 Wilson ++
	if (pid == 0xC001 )
	{
	    pid = 0xC004;
	    dev->functions = 0x612;
