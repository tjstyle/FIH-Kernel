...
#if 0 // these configs are for F series
#define USBSET 123
#define CHR_EN 33
#define CHR_1A 26
#endif

//FIHTDC, add for 5310 battery charging, MayLi, 2010.05.06 {+
#define GPIO_OTG_EN          29	
#define GPIO_USB_2A_DET_n    41          
extern void Battery_update_state(int _device, int device_state);
extern void SetChargingCurrent(int iCurrent);
//FIHTDC, add for 5310 battery charging, MayLi, 2010.05.06 -}
extern int g_otg_status;//OTG_ststus: 0=out,1=in
#endif
...
//FIHTDC, Adds for 5310 battery charging, MayLi, 2010.05.06 {+
	if ((readl(USB_PORTSC) & PORTSC_LS) == PORTSC_LS)
	{
		pr_info("\n*********** Charger Type: WALL CHARGER\n\n");
#ifdef CONFIG_FIH_FXX		
		if(!gpio_get_value(GPIO_USB_2A_DET_n)) //if low, needs 2A
		{
			pr_info("AC:Set the charging current to 2000mA\n");
			SetChargingCurrent(2000);
		}
		else
		{
			pr_info("AC:Set the charging current to 1000mA\n");
			SetChargingCurrent(1000);
		}
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	if (temp == USB_CHG_TYPE__WALLCHARGER) {
		...
		/* FIH, Michael Kao, 2010/07/23{ */
		/* [FXX_CR], Slove AC plug in but system not wake up issue*/
		//wake_unlock(&ui->wlock);
		wake_lock_timeout(&ui->wlock, 3 * HZ );
		/* FIH, Michael Kao, 2010/07/23{ */
	}
}
...
//add for MR7 charging
	gpio_direction_output(GPIO_OTG_EN,0);
	
	gpio_direction_input(GPIO_USB_2A_DET_n);
//add for MR7
	...
	for (;;) {
		...
		case USB_STATE_IDLE:
			if (flags & USB_FLAG_START) {
				...
				usb_reset(ui);
/* FIH, WilsonWHLee, 2010/07/07 { */

/* [FM6E.B-471], OTG device cannot work after plug in and out USB data cable */
...
/* }FIH, WilsonWHLee, 2010/07/07 */
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
				...
				msm72k_pm_qos_update(0);
				/* FIH, Michael Kao, 2010/07/23{ */
				/* [FXX_CR], Slove charger plug out but system not wake up issue*/
				//msleep(500);
...
/*current protection indication*/
			else if(check_USB_type == 1 || check_USB_type == 3)  
/* }FIH, WilsonWHLee, 2010/06/15 */          
       	     val->intval = USB_Connect;
       else	 
             val->intval = 0;    
...
//		      struct usb_info *ui = the_usb_info;
  	...
  	if (!strncmp(buf, "diag", strlen("diag"))) {
	     ...
	     //fih_printk (msm_hsusb_debug_mask, FIH_DEBUG_ZONE_G0, 
	    // "%s: msm_hsusb_store_change_pid \n", __func__);
	    ...
	    // "%s: msm_hsusb_store_change_pid \n", __func__);	
		  ...
		  //fih_printk (msm_hsusb_debug_mask, FIH_DEBUG_ZONE_G0, 
		 // "%s:  msm_hsusb_store_change_pid failed buf==%s\n", __func__,buf);
...
/* } FIH, WilsonWHLee, 2009/09/021  */
/* FIH, WilsonWHLee, 2010/10/18 { */
/* [FXX_CR], Virtual file for disabling USB controller */
static ssize_t msm72k_turnoffUSB(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
    	struct usb_info *ui = the_usb_info;
...
/* FIH, WilsonWHLee, 2010/10/18 { */
/* [FXX_CR], Virtual file for disabling USB controller */
static DEVICE_ATTR(turnoffUSB, 0664,msm72k_turnoffUSB, NULL);
/* } FIH, WilsonWHLee, 2010/10/18  */
...
void usb_chg_pid(bool usb_switch) 
{
  ...
  if(usb_switch){
   ...
   //      power_supply_changed(g_ps_usb);
   //   }
    if(fih_enable == 3 && g_otg_status == 1){
        // upid = 0xc003;
         USB_Connect = 8;
		 g_ps_usb->changed = 1;
         power_supply_changed(g_ps_usb);
...
/* FIH, WilsonWHLee, 2010/10/18 { */
/* [FXX_CR], Virtual file for disabling USB controller */
    retval = device_create_file(&ui->gadget.dev, &dev_attr_turnoffUSB);
	if (retval != 0)
		INFO("failed to create sysfs entry: (dev_attr_turnoffUSB) error: (%d)\n",
					retval);
/* } FIH, WilsonWHLee, 2010/10/18  */	
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	device_remove_file(&dev->gadget.dev, &dev_attr_switch_enable);
	device_remove_file(&dev->gadget.dev, &dev_attr_switch);
	device_remove_file(&dev->gadget.dev, &dev_attr_hs_connect);
