...
static unsigned long rtcalarm_time;
...
#ifdef CONFIG_FIH_FXX
    static uint32_t rtc_debug_mask = 0;
    module_param_named(
        debug_mask, rtc_debug_mask, uint, S_IRUGO | S_IWUSR | S_IWGRP
    );
    #ifdef pr_err
        #undef  pr_err
    #endif
    ...
    #define pr_err(fmt, ...)    \
        do  \
        {   \
            fih_printk(rtc_debug_mask, FIH_DEBUG_ZONE_G0, KERN_ERR fmt, ##__VA_ARGS__);\
        }while(0)

    #define pr_info(fmt, ...)    \
        do  \
        {   \
            fih_printk(rtc_debug_mask, FIH_DEBUG_ZONE_G0, KERN_INFO fmt, ##__VA_ARGS__);\
        }while(0)

    #define pr_debug(fmt, ...)    \
        do  \
        {   \
            fih_printk(rtc_debug_mask, FIH_DEBUG_ZONE_G0, KERN_DEBUG fmt, ##__VA_ARGS__);\
        }while(0)
#endif
struct rtc_tod_args {
	int proc;
...
/* [FXX_CR], bld build system */
	...
	int RPC_wakeup_cycle_time=600;  /*FIHTDC, MayLi adds for battery updates per 10 minutes in suspend mode, 2010.08.20*/
#endif
/* } FIH, Charles Huang, 2010/09/16 */
	if (rtcalarm_time) {
...
/* [FXX_CR], bld build system */
#ifdef CONFIG_FIH_PROJECT_FM6
		/*FIHTDC, MayLi adds for battery updates per 10 minutes in suspend mode, 2010.08.21 {+*/
...
/* [FXX_CR], bld build system */
#ifdef CONFIG_FIH_PROJECT_FM6
	/*FIHTDC, MayLi adds for battery updates per 10 minutes in suspend mode, 2010.08.20 {*/
	{
