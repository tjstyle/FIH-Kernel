...
#define FIH_DEBUG_CFG_DATA_LEN 15 //SW2-5-1-BH-DbgCfgTool-02+
...
static int __init dbgcfgtool_init(void)
{
    ...
    //Div6-PT2-Core-BH-DbgCfgToolPorting-01+]
    unsigned int i = 0; // SW2-5-1-BH-DbgCfgTool-02+
    ...
    if( ret == 0 )
    {
        ...
        {
            //ret = -1; // SW2-5-1-BH-DbgCfgTool-02-
            ...
            printk(KERN_ERR "[dbgcfgtool] reset to default!\n");
	    for(i=0; i<FIH_DEBUG_CFG_DATA_LEN;i++) {
	        localcfg[i] = 0;
	    }
	    localcfg[i] = 0xff;
		...
		pointer_temp = (unsigned int *)localcfg;	
            printk(KERN_INFO "[dbgcfgtool] tempcfg: 0x%08x 0x%08x 0x%08x 0x%08x\n", pointer_temp[0], pointer_temp[1], pointer_temp[2], pointer_temp[3]); //SW2-5-1-MP-DbgCfgTool-01+                        
            // SW2-5-1-BH-DbgCfgTool-02+ ]
        }
