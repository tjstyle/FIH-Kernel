...
/* [F0X_CR], add power collapse event log */
...
module_param_named(
	debug_mask, msm_pm_debug_mask, uint, 0644);
#else
...
/* [F0X_CR], add power collapse event log */
    ...
    if(!from_idle) {
        ...
        if(cpu_loading_debug_get() == true) {
            smsm_event_sleep_info(msm_pm_smem_data->wakeup_reason);
        }
    }
#endif
/* } FIH, SimonSSChang, 2009/12/15 */    
...
/* } FIH, SimonSSChang, 2009/12/15 */
	//[+++]Add for fast dormancy
	#ifdef CONFIG_FIH_FXX        
	static struct file *dorm_fifo = NULL;
	static char dorm_input[1];
	#endif
    	//[---]Add for fast dormancy
#ifdef CONFIG_MSM_IDLE_STATS
	DECLARE_BITMAP(clk_ids, NR_CLKS);
