...
/* FIH, KennyChu, 2010/01/04, add debug mask {*/
#ifdef CONFIG_FIH_FXX
static uint32_t vibrator_debug_mask = 0;
#endif
/* } FIH, KennyChu, 2010/01/04*/
...
static void set_pmic_vibrator(int on)
{
	...
	if (!vib_endpoint) {
		...
		if (IS_ERR(vib_endpoint)) {
            ...
            fih_printk(vibrator_debug_mask, FIH_DEBUG_ZONE_G0, "init vib rpc failed!\n");
			vib_endpoint = 0;
			return;
		}
	}
...
static int pmic_vibrator_off_thread(void *arg)
{
    ...
    fih_printk(vibrator_debug_mask, FIH_DEBUG_ZONE_G0, "vib_off_thread running\n");
    ...
    while (1) {        
        ...
        fih_printk(vibrator_debug_mask, FIH_DEBUG_ZONE_G0, "Got complete signal\n");
...
/* FIH, KennyChu, 2009/07/23, solve vibration can't be stopped issue{ */
    ...
    fih_printk(vibrator_debug_mask, FIH_DEBUG_ZONE_G0, "TIMER canceled\n");
    ...
    if (value != 0) {
        ...
        fih_printk(vibrator_debug_mask, FIH_DEBUG_ZONE_G0, "execute pmic_vibrator_on\n");
        ...
        if (hrtimer_active(&vibe_timer))
        {
            ...
            fih_printk(vibrator_debug_mask, FIH_DEBUG_ZONE_G0, "TIMER running\n");
        }
    ...
    else
    {
        ...
        fih_printk(vibrator_debug_mask, FIH_DEBUG_ZONE_G0, "stop vibrator directly\n");
        mutex_lock(&vibrator_lock);
        set_pmic_vibrator(0);
        mutex_unlock(&vibrator_lock);
...
/* FIH, KennyChu, 2010/01/04, add debug mask {*/
#ifdef CONFIG_FIH_FXX
module_param_named(debug_mask, vibrator_debug_mask, uint, 0644);
#endif
/* } FIH, KennyChu, 2010/01/04*/
