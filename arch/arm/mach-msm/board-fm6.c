...
#else
#include <linux/panjit_i2c.h>  //Added for capacitive touch panel, by Stanley++ 2009/05/20
...
#else
static struct panjit_i2c_platform_data panjit_device = {
    .abs_x_min= 0,
    .abs_x_max= 800,    //Terry
    .abs_y_min= 0,
    .abs_y_max= 480,    //Terry
...
/* [FXX_CR], change backlight driver from max7302 to max8831 */
    ...
    //FIHTDC, adds for register gas gauge, MayLi, 2010.08.20 {+
	{
		I2C_BOARD_INFO("ds2786", 0x6c>>1),
	},
    //FIHTDC, adds for register gas gauge, MayLi, 2010.08.20 -}
#endif
/* } FIH, Neo Chen, 2009/07/02 */
...
#else
    {
        I2C_BOARD_INFO("panjit", 0x27), //Terry
        .irq            = MSM_GPIO_TO_INT(39), //Terry
        .platform_data  = &panjit_device,
    },
#endif
//Added for capacitive touch panel, by Stanley-- 2009/06/03
...
static struct platform_device msm_batt_device = {
	.name 		    = "goldfish-battery",   /*FIHTDC, MayLi changes it, 2010.08.20*/
	.id		    = -1,
...
static unsigned sdcc_cfg_data[][6] = {
	...
	GPIO_CFG(93, 1, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_12MA),
	},
	/* SDC4 configs */
	{
...
/* FIH, MonkyChen, 2010/05/10 */
/* Modify pulldown configuration array size (n=4) to be suitable for normal configuration array*/
/* Correct arrary size will prevent from unpredictable erros occurred*/
/* However, we don't use SDC2 and SDC4 pull down configuration for the FM6 project*/
#if CONFIG_FIH_FXX
static unsigned sdcc_cfg_pulldown[][6] = {
	...
	//++ FM6F.B-197,  Increase clock driving strength for SD card compatibility
	...
	/* SDC2 configs */
	{
	GPIO_CFG(62, 2, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(63, 2, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(64, 2, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(65, 2, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(66, 2, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(67, 2, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	},
	/* SDC3 configs */
	{
	GPIO_CFG(88, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(89, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(90, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(91, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(92, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	//++ FM6F.B-197,  Increase clock driving strength for SD card compatibility
	GPIO_CFG(93, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_12MA),
	//-- FM6F.B-197
	...
	/* SDC4 configs */
	{
	GPIO_CFG(19, 3, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(20, 3, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(21, 4, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(107, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(108, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	GPIO_CFG(109, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA),
	}
};
#endif	

/*2010/06/02, FIH,AustinCWTzeng, separate vreg_sts variable to make it independent  for sdc1 and sdc3*/
static unsigned long /*vreg_sts,*/vreg_sts_sdc1 = 0,vreg_sts_sdc3 = 0, gpio_sts;
static unsigned mpp_mmc = 2;

/*2010/07/13,AustinCWTzeng{*/
static struct vreg *vreg_mmc;//for FM6 sdc1 power enable pin
static unsigned mpp_mmc_21 = 21-1;//for FN6_PR1 sdc1 power enable pin 
static unsigned mpp_mmc_3 = 3-1;//for FN6_PR2 sdc1 power enable pin 
/*}2010/07/13,AustinCWTzeng*/

static struct vreg *vreg_gp5; /*2010/05/10, MonkyChen, Add for GP5 configuration*/
...
static void msm_sdcc_setup_gpio(int dev_id, unsigned int enable)
{
	...
	if (enable)
	{
		...
		printk(KERN_INFO"%s: sdcc_%d pull up\n",__func__, dev_id);
	}
	else
	{
		if (1 == dev_id || 3 == dev_id){ //monky: Only pulldown sdcc1 or sdcc3
			...
			printk(KERN_INFO"%s: sdcc_%d pull down\n",__func__, dev_id);
		}
	}
	...
	printk(KERN_INFO"%s: cfg_data_num = %d,(pull down)\n",__func__, dev_id-1);
	#endif
...
static uint32_t msm_sdcc_setup_power(struct device *dv, unsigned int vdd)
{
	...
	printk(KERN_INFO"++ %s, vdd=%d, id=%d\n", __func__, vdd, pdev->id);			
	
	/* FIH, BillHJChang, 2009/07/22 { */
	/* [FXX_CR], 7627 SD card power supply source changed*/
	#ifdef CONFIG_FIH_FXX
	/*FIH, MonkyChen, 2010/05/10 { */
	/*get different power source of SDC for FM6 project*/
	if (1 == pdev->id){ // sdc1	
	
       /*FIH,AustinCWTzeng,2010/05/31{*/
	/*SDC1 power source has changed */
	/*vreg_mmc for FM6 and vpp_mmc_21 for FN6*/
	if ((HWID >= CMCS_FN6_PR1) && ((HWID <= CMCS_FN6_MP1))){	
		
		/*FIH,AustinCWTzeng,2010/07/13{*/
		/*Choose correct FN6 HWID  device to control SD power.*/
              switch(HWID){

                   case CMCS_FN6_PR1:
				   	printk(KERN_INFO"%s: Config mpp_mmc_21 for External SD power !\n", __func__);		
				   	break;
					
		     case CMCS_FN6_PR2:
			 	       printk(KERN_INFO"%s: Config mpp_mmc_3 for External SD power !\n", __func__);	
			 	       break;
	      ...
	      /*}FIH,AustinCWTzeng,2010/07/13*/ 
			...
			if (vreg_mmc == NULL){
						...
						if (IS_ERR(vreg_mmc))
							printk(KERN_ERR "%s: vreg_mmc get failed (%ld)\n",__func__, PTR_ERR(vreg_mmc));			
					}
	}
       /*}FIH,AustinCWTzeng,2010/05/31*/
	...
	if (3 == pdev->id){	// sdc3
		if (vreg_gp5 == NULL){
			...
			if (IS_ERR(vreg_gp5))
				printk(KERN_ERR "%s: vreg_gp5 get failed (%ld)\n",__func__, PTR_ERR(vreg_gp5));			
		}
	}
	/* } FIH, MonkyChen, 2010/05/10 */
	#endif	
	...
	/* FIH, MonkyChen, 2010/05/10 {*/
	/* Need more care on the sequence of setup GPIO and SD Vdd */
	/* Based on the different HW design, */
	/* to setup gpio earilier than turn on power may cause abnormal situation */
	
	msm_sdcc_setup_gpio(pdev->id, !!vdd);	
	
	if (vdd == 0) {
		
		/* FIH, AustinCWTzeng, 2010/06/02 { */
		/* Fixed SD power consumption issue.*/
		/* SDC1 or SDC3 power pin was enabled by SD driver and control two flags*/
		/*that is vreg_sts_sdc1 and vreg_sts_sdc1 */
		/*if (!vreg_sts)
			return 0;

		clear_bit(pdev->id, &vreg_sts);*/ 

		/*if (!vreg_sts) */if(vreg_sts_sdc1 !=0 || vreg_sts_sdc3!=0){
			if (machine_is_msm7x27_ffa()) {
				...
				printk(KERN_INFO"%s: [sts_sdc1= 0x%x][sts_sdc3= 0x%x]\r\n",__func__,(int)vreg_sts_sdc1,(int)vreg_sts_sdc3);	

				if (1 == pdev->id && vreg_sts_sdc1!=0){
					clear_bit(pdev->id, &vreg_sts_sdc1);
					...
					if ((HWID >= CMCS_FN6_PR1) && ((HWID <= CMCS_FN6_MP1))){	
						...
						case CMCS_FN6_PR1:
										...
										/*FIH,AustinCWTzeng,2010/07/13{*/
										/*Change power source to mpp_mmc_21 for FN6_PR1*/
										rc = mpp_config_digital_out(mpp_mmc_21,
										MPP_CFG(MPP_DLOGIC_LVL_MSMP,
										MPP_DLOGIC_OUT_CTRL_LOW));
										/*}FIH,AustinCWTzeng,2010/07/13*/	
							break;
						...
						case CMCS_FN6_PR2:
										...
										/*FIH,AustinCWTzeng,2010/7/13{*/
										/*Change power source to mpp_mmc_3 for FN6_PR2*/
										rc = mpp_config_digital_out(mpp_mmc_3,
										MPP_CFG(MPP_DLOGIC_LVL_MSMP,
										MPP_DLOGIC_OUT_CTRL_LOW));
										/*}FIH,AustinCWTzeng,2010/07/13*/
						       break;
						...
						default:
										...
										/*FIH,AustinCWTzeng,2010/07/13{*/
										/*Change power source to mpp_mmc_3 for another FN6 types*/
										rc = mpp_config_digital_out(mpp_mmc_3,
										MPP_CFG(MPP_DLOGIC_LVL_MSMP,
										MPP_DLOGIC_OUT_CTRL_LOW));
										/*}FIH,AustinCWTzeng,2010/07/13*/   
						...
						/*}FIH,AustinCWTzeng,2010/07/13*/ 
					       ...
					       /*FIH,AustinCWTzeng,2010/05/31{*/
						/*Change power source to vreg_mmc for FM6*/
					...
					#ifdef CONFIG_FIH_FXX
					printk(KERN_INFO"%s: [SD%d card power: off]\r\n", __func__,pdev->id );	
					#endif
					/* } FIH, BillHJChang, 2009/07/15 */		
				
				}else if (3 == pdev->id && vreg_sts_sdc3!=0){
				      clear_bit(pdev->id, &vreg_sts_sdc3);
			...
			if (rc)
				printk(KERN_ERR "%s: vreg_disable return val: %d \n",
					__func__, rc);
		}
	    /* }FIH, AustinCWTzeng, 2010/06/02 */
		return 0;
	...
	/*if (!vreg_sts)*/if (vreg_sts_sdc1==0 || vreg_sts_sdc3==0) {
		if (machine_is_msm7x25_ffa() || machine_is_msm7x27_ffa()) {
			...
			/* FIH, MonkyChen, 2010/05/10 { */
		    /* Setup different voltage level by device id */
			if (1 == pdev->id && vreg_sts_sdc1 ==0){ //sdc1
				set_bit(pdev->id, &vreg_sts_sdc1);
				...
				if ((HWID >= CMCS_FN6_PR1) && ((HWID <= CMCS_FN6_MP1))){	
					...
					/*Choose correct FN6 HWID  device to control SD power.*/
					      switch(HWID){
					           ...
					           case CMCS_FN6_PR1:
											...
											/*FIH,AustinCWTzeng,2010/07/13{*/
											/*Change power source to MMP21 for FN6_PR1*/
											rc = mpp_config_digital_out(mpp_mmc_21,
											MPP_CFG(MPP_DLOGIC_LVL_MSMP,
											MPP_DLOGIC_OUT_CTRL_HIGH));
											printk(KERN_INFO"%s: [SD card power mpp_mmc_21 enable: on]\r\n",__func__);	
											/*}FIH,AustinCWTzeng,2010/05/31*/	
							   	break;
					          ...
					          case CMCS_FN6_PR2:
											...
											/*FIH,AustinCWTzeng,2010/07/13{*/
											/*Change power source to MMP3 for FN6_PR2*/
											rc = mpp_config_digital_out(mpp_mmc_3,
											MPP_CFG(MPP_DLOGIC_LVL_MSMP,
											MPP_DLOGIC_OUT_CTRL_HIGH));
											printk(KERN_INFO"%s: [SD card power mpp_mmc_21 enable: on]\r\n",__func__);	
						   ...
						   default:
											/*FIH,AustinCWTzeng,2010/07/13{*/
											/*Change power source to MMP3 for for another FN6 types*/
											rc = mpp_config_digital_out(mpp_mmc_3,
											MPP_CFG(MPP_DLOGIC_LVL_MSMP,
											MPP_DLOGIC_OUT_CTRL_HIGH));
											printk(KERN_INFO"%s: [SD card power mpp_mmc_21 enable: on]\r\n",__func__);	
				    ...
				    /*FIH,AustinCWTzeng,2010/05/31{*/
				    /*Change power source to vreg_mmc for FM6*/
				   if (!vreg_set_level(vreg_mmc, 2850))
				...
				/* [FXX_CR], add debug message for SD card power supply */
				#ifdef CONFIG_FIH_FXX
				printk(KERN_INFO"%s: [SD%d card power: on]\r\n",__func__,pdev->id);	
				#endif
				/* } FIH, BillHJChang, 2009/07/15 */
			}
			else if (3 == pdev->id && vreg_sts_sdc3 ==0){ //sdc3
				set_bit(pdev->id, &vreg_sts_sdc3);

				if (!vreg_set_level(vreg_gp5, 2850))
		...
		if (rc)
			printk(KERN_ERR "%s: vreg_enable return val: %d \n",
					__func__, rc);
	}
