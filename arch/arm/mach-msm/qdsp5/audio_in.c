...
static int audio_dsp_set_ns(struct audio_in *audio)
{
	...
	if (audio->ns_enable) {
		...
		audio->ns_cfg.ec_mode_new =
        	...
        	MM_INFO("audio_dsp_set_ns\n");
			MM_INFO("ec_mode_new = 0x%x, dens_limit_ns = 0x%x\n", audio->ns_cfg.ec_mode_new, audio->ns_cfg.dens_limit_ns);
	} else {
		audio->ns_cfg.ec_mode_new =
...
static int audio_dsp_set_iir(struct audio_in *audio)
{
	...
	if (audio->iir_enable){
		...
		MM_INFO("audio_dsp_set_iir\n");
		MM_INFO("filter1_msw = 0x%x, filter1_lsw = 0x%x, selected2 = 0x%x\n", audio->iir_cfg.numerator_coeff_b0_filter1_msw, audio->iir_cfg.denominator_coeff_a0_filter1_lsw, audio->iir_cfg.channel_selected2);
	}
	else
...
static long audpre_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_ENABLE_AUDPRE:
		...
		MM_INFO("AUDIO_ENABLE_AUDPRE mask = 0x%x\n", enable_mask);
