...
/* FIH, NicoleWeng, @20100506 { */	//handle debounce time here and remove debonce time in work queue
	fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"Q7x27_kybd_irqhandler, time :%d\n", jiffies_to_msecs(jiffies - lastTime));
	if(lastTime!=0 && time_after(lastTime+KBD_DEBOUNCE_TIME, jiffies)){
		fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"Q7x27_kybd_irqhandler filter of, time :%d\n", jiffies_to_msecs(jiffies - lastTime));		
		return IRQ_HANDLED;
	}	
	lastTime = jiffies;
/* } FIH, NicoleWeng, @20100506  */	
...
int Q7x27_kybd_proximity_irqsetup(void)
{
	if(kpdev)
	{
		...
		fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH:Q7x27_kybd_proximity_irqsetup keypress KEY_RINGSWITCH = %d\n",KEY_LEFTSHIFT);
		...
		fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH:Q7x27_kybd_proximity_irqsetup keyrelease KEY_RINGSWITCH = %d\n",KEY_LEFTSHIFT);
		input_sync(kpdev);
		return 0;
	}
	//printk(KERN_INFO "FIH:Q7x27_kybd_proximity_irqsetup does nothing.\n");
	fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH:Q7x27_kybd_proximity_irqsetup does nothing.\n");
	return 0;
...
/* } FIH, PeterKCTseng, @20090527 */
	...
	if (rc) {
		...
		//       "pin %d (rc=%d)\n", kbd_key_1_pin, rc);
		fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"gpio_direction_input failed on %d (rc=%d)\n",kbd_key_1_pin, rc);
		goto err_gpioconfig;
	}
	...
	if (rc) {
		...
		//       "pin %d (rc=%d)\n", kbd_key_2_pin, rc);
		fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"gpio_direction_output failed on pin %d (rc=%d)\n",kbd_key_2_pin, rc);
		goto err_gpioconfig;
	}
        ...
        if(SuspendState == PM_SUSPEND_MEM)//3
        {
            if(idev)
            {
            	input_report_key(idev, KEY_END, KBD_IN_KEYPRESS);
               		...
               		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_END\n");
            	
            	input_report_key(idev, KEY_END, KBD_IN_KEYRELEASE);
               		printk(KERN_INFO "FIH: keyrelease KEY_END\n");
               		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease KEY_END\n");
            	input_sync(idev);
            }
        }
        //-----------------------------------------------------------------misty
        ...
        if(SuspendState == PM_SUSPEND_MEM)
        {
            if(idev)
            {
            	input_report_key(idev, KEY_SEND, KBD_IN_KEYPRESS);
            		...
            		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_SEND\n");
            	input_report_key(idev, KEY_SEND, KBD_IN_KEYRELEASE);
            		printk(KERN_INFO "FIH: keyrelease KEY_SEND\n");
            		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease KEY_SEND\n");
            	input_sync(idev);
            }
        }
        //-----------------------------------------------------------------misty
...
#endif
/* } FIH, PeterKCTseng, @20090520 */
	//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"[Misty]KEY 1 <%d>\n", key_1_val);
	//printk(KERN_INFO "KEY 1 <%d>\n", key_1_val);
...
//FIH, NicoleWeng, 2010/05/26 for F0XE.B-955 {
                ...
                {
                		//  FIH, RogerCCYen, 2010/09/07 for FAA 6030 F image
                		if( g_ORIGHWID >=CMCS_125_4G4G_FAA_PR1 && g_ORIGHWID<= CMCS_128_4G4G_FAA_MP1)
                		{
                			input_report_key(idev, KEY_BACK, KBD_IN_KEYPRESS);
                		    //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_SEND\n");
                			printk(KERN_INFO "FIH: keypress KEY_BACK\n");
            				input_report_key(idev, KEY_BACK, KBD_IN_KEYRELEASE);
                		    //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease KEY_SEND\n");
                			printk(KERN_INFO "FIH: keyrelease KEY_BACK\n");
            				input_sync(idev);                		
                		}
                		else
                		{ 
...
//FIH, NicoleWeng, 2010/05/26 for F0XE.B-955 {
                ...
                {
                		//  FIH, NicoleWeng, 2010/08/14 for F0XF.B-216
                		if( g_ORIGHWID >= CMCS_125_4G4G_FAA_PR1 && g_ORIGHWID <=  CMCS_128_4G4G_FAA_MP1)
                		{
                			if (state) 
            				{
...
#endif
/* } FIH, PeterKCTseng, @20090520 */
    //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"[Misty]KEY 2 <%d>\n", key_2_val);
	//printk(KERN_INFO "KEY 2 <%d>\n", key_2_val);
    ...
    if(EnableKeyInt)
    {
        ...
        if(SuspendState == PM_SUSPEND_MEM)//3
        {
            if(idev)
            {
                ...
                if( g_ORIGHWID>=CMCS_125_4G4G_FAA_PR1 && g_ORIGHWID<= CMCS_128_4G4G_FAA_MP1)
                {
                		input_report_key(idev, KEY_MENU, KBD_IN_KEYPRESS);
                    //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_SEND\n");
                		printk(KERN_INFO "FIH: keypress KEY_MENU\n");
            			input_report_key(idev, KEY_MENU, KBD_IN_KEYRELEASE);
                    //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease KEY_SEND\n");
                		printk(KERN_INFO "FIH: keyrelease KEY_MENU\n");
            			input_sync(idev);                		
                }
                else
                { 
    ...
    if(EnableKeyInt)
    {
        if(idev)
        {
        	if (state) {
        		...
        		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_CAMERA\n");
        	} else {
        		input_report_key(idev, KEY_CAMERA, KBD_IN_KEYRELEASE); //report KEY CAMERA releasing
        		printk(KERN_INFO "FIH: keyrelease KEY_CAMERA\n");		
        		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease KEY_CAMERA\n");
        	}
    ...
    if(EnableKeyInt)
    {
        if(idev)
        {
        	if (state) {
        		...
        		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_FOCUS\n");
        	} else {
        		input_report_key(idev, KEY_FOCUS, KBD_IN_KEYRELEASE); //report KEY FOCUS releasing
        		printk(KERN_INFO "FIH: keyrelease KEY_FOCUS\n");		
        		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease KEY_FOCUS\n");
        	}
...
//+++++misty tmp mark    
//#if 0
//KC-d?-        if(input_jogball_exist()==1)//F910/F911 jogball
        if(0)//KC-d?+
        {
            if(idev)
             {
            	if (state) {
            		...
            		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress SELECT_INPUT_ENTER= %d\n", SELECT_INPUT_ENTER);
            	} else {
            		...
            		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease SELECT_INPUT_ENTER= %d\n", SELECT_INPUT_ENTER);
            	}
            ...
            if(idev)
            {
                if(SuspendState == PM_SUSPEND_MEM)
                {
                    ...
                    else
                    {
                        ...
                        //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_Center(HOME)\n");
                        input_report_key(idev, SELECT_INPUT_BTN, KBD_IN_KEYRELEASE);
                        printk(KERN_INFO "FIH: keyrelease KEY_Center(HOME)\n");
                        //	fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease KEY_Center(HOME)\n");
                        input_sync(idev);    
                    }
                ...
                else
                {
                    if((g_HWID == CMCS_RTP_PR2))
                    {
                    	if (state) {
                    		...
                    		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress SELECT_INPUT_MENU= %d\n", SELECT_INPUT_MENU);
                    	} else {
                    		...
                    		//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease SELECT_INPUT_MENU= %d\n", SELECT_INPUT_MENU);
                    	}
                    	input_sync(idev);
                    }
                    else
                    {
                        if (state) {
                		    ...
                		    //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress SELECT_INPUT_BTN= %d\n", SELECT_INPUT_BTN);
                	    } else {
                		    ...
                		    //fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keyrelease SELECT_INPUT_BTN= %d\n", SELECT_INPUT_BTN);
                	    }
...
// }  FIH, RogerCCYen, 2010/09/07 for FAA 60 30 F image
...
// } FIH, NicoleWeng, 2010/05/26 for F0XE.B-955
       {
           ...
           fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"enable key1   wakeup pin: %d\n",rd->key_1_pin);
    	   enable_irq_wake(MSM_GPIO_TO_INT(rd->key_1_pin));
           b_Key1_EnableWakeIrq = true;
    
           //printk(KERN_INFO "enable key2   wakeup pin: %d\n", rd->key_2_pin);  
           fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"enable key2   wakeup pin: %d\n", rd->key_2_pin);    
       ...
       if(g_centerkey)
       {
           ...
           fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"diable center interrupt pin: %d\n", g_center_pin);
...
// } FIH, NicoleWeng, 2010/05/26 for F0XE.B-955
      {
          ...
          fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"diable key1 interrupt pin: %d\n",rd->key_1_pin);  
          disable_irq(MSM_GPIO_TO_INT(rd->key_1_pin));
          b_Key1_DisableIrq = true;
         fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"diable key2 interrupt pin: %d\n",rd->key_2_pin);
...
// printk(KERN_ERR "%s""#######################g_center_pin:%d \n", __func__,g_center_pin);
 ...
 if (device_may_wakeup(&rd->pdev->dev))
 {
   ...
   fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"b_Key1_EnableWakeIrq = %d\n", b_Key1_EnableWakeIrq);
   if(b_Key1_EnableWakeIrq)
   {
     //printk(KERN_INFO "disable key1   wakeup pin: %d\n", rd->key_1_pin);
       fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"disable key1   wakeup pin: %d\n",rd->key_1_pin);
   ...
   //printk(KERN_INFO "b_Key2_EnableWakeIrq = %d\n", b_Key2_EnableWakeIrq);
    fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"b_Key2_EnableWakeIrq = %d\n",b_Key2_EnableWakeIrq);
   if(b_Key2_EnableWakeIrq)
   {
     //printk(KERN_INFO "disable key2   wakeup pin: %d\n", rd->key_2_pin);
      fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"disable key2   wakeup pin: %d\n", rd->key_2_pin);
 ...
 fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"b_Key1_DisableIrq = %d\n", b_Key1_DisableIrq);
 if(b_Key1_DisableIrq)
 {
   //printk(KERN_INFO "enable key1 interrupt   pin: %d\n", rd->key_1_pin);
    fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"enable key1 interrupt   pin: %d\n", rd->key_1_pin);
   enable_irq(MSM_GPIO_TO_INT(rd->key_1_pin)); 
   b_Key1_DisableIrq = false; 
 }

 //printk(KERN_INFO "b_Key2_DisableIrq = %d\n", b_Key2_DisableIrq);
  fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"b_Key2_DisableIrq = %d\n",b_Key2_DisableIrq);
 if(b_Key2_DisableIrq)
 {
   //printk(KERN_INFO "enable key2 interrupt   pin: %d\n", rd->key_2_pin);
    fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"enable key2 interrupt   pin: %d\n", rd->key_2_pin);
   enable_irq(MSM_GPIO_TO_INT(rd->key_2_pin));  
   b_Key2_DisableIrq = false; 
 }

 //printk(KERN_INFO "b_Center_DisableIrq = %d\n", b_Center_DisableIrq); 
 fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"b_Center_DisableIrq = %d\n",b_Center_DisableIrq);
 if(b_Center_DisableIrq)
 {
   //printk(KERN_INFO "enable center interrupt pin: %d\n", g_center_pin);
   fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"enable center interrupt pin: %d\n",g_center_pin);
...
/* The active type of input pin   */
    ...
    if (g_HWID  == CMCS_HW_VER_EVB1) {
		...
		fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: CMCS_HW_VER_EVB1, g_HWID= %d\n", g_HWID);
...
/* } FIH, PeterKCTseng, @20090603 */
	...
	if (!headset_hook_wq) {
		fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,  "%s: create workque failed \n", __func__);		
		return -EBUSY;
	}
	/* } FIH, Karen Liao, 2010/05/05 */
...
//	rd->center_pin     = setup_data->center_pin;
    ...
    fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"g_ORIGHWID:0x%x\n",g_ORIGHWID);
		...
		if(!(g_ORIGHWID >= CMCS_125_FST_PR1 &&  g_ORIGHWID <= CMCS_128_FST_MP1) 
		  ...
		  && !(g_ORIGHWID >= CMCS_128_4G4G_FAA_PR1 &&  g_ORIGHWID <= CMCS_128_4G4G_FAA_MP1))
			...
			if(gpio_get_value(GPIO_NEG_Y))
			{
				...
				fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"OFF center key\n");
			}
		}
	}
...
//  FIH, NicoleWeng, 2010/05/26 for F0XE.B-955 {          
           if(g_ORIGHWID == CMCS_125_FST_MP1)
           {
	   	 ...
	   	 //g_SndEndkey=false;
	   // printk(KERN_ERR "OFF SND/END key\n");
	   	//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"OFF SND/END key\n");
// } FIH, NicoleWeng, 2010/05/26 for F0XE.B-955     
	}
	
	gpio_tlmm_config(GPIO_CFG(rd->center_pin, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);
#endif
/* } FIH, PeterKCTseng, @20090527 */
...
failexit2:
    //printk(KERN_INFO "FIH: error out failexit2\n");
    fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: error out failexit2\n");
	free_irq(MSM_GPIO_TO_INT(rd->key_1_pin), rd);
	free_irq(MSM_GPIO_TO_INT(rd->key_2_pin), rd);
